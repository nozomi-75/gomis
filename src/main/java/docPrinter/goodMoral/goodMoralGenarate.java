...
// a class to generate a good moral
public class goodMoralGenarate {
	private static final Logger logger = LogManager.getLogger(goodMoralGenarate.class);

	public goodMoralGenarate() {
	}

	/**
	 * Generates a Good Moral Certificate by merging a map of values with a DOCX template.
	 ...
	 */
	public boolean generateGoodMoralDocx(Student student, File outputFolder, File templateFile, Map<String, String> values, String action) {
		try {
			...
			logger.error("Error generating good moral certificate", e);
			return false;
		}
	}

	private String generateFileName(Student student) {
		String baseName = student.getStudentLastname() + "_" + student.getStudentFirstname() + "_GoodMoral";
