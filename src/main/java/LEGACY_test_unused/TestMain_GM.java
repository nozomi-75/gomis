...
public class TestMain_GM extends JFrame {
	...
	private goodMoralGenarate generator = new goodMoralGenarate();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(() -> {
			try {
				...
				TestMain_GM frame = new TestMain_GM(conn);
	...
	public TestMain_GM(Connection connection) {
		this.studentsDataDAO = new StudentsDataDAO(connection);
		
		setTitle("Good Moral Certificate - Test Generator");
		...
		JButton printSelectedBtn = new JButton("Generate Certificates");
	...
	private void initializeDefaultPaths() {
		...
		templateFile = templateManager.getActiveTemplate(templateManager.TemplateType.GOOD_MORAL);
		...
		if (!templateFile.exists()) {
			JOptionPane.showMessageDialog(
				this,
				"No active template found for Good Moral. Please load or import a template using the Template Manager.",
				"Missing Template",
	...
	private void validateTemplate() {
		boolean isValid = templateManager.validateTemplate(templateManager.TemplateType.GOOD_MORAL);
		String status = templateManager.getTemplateStatus(templateManager.TemplateType.GOOD_MORAL);
	...
	private void resetTemplate() {
		int result = JOptionPane.showConfirmDialog(this,
			"Are you sure you want to reset the Good Moral template to default?",
			"Reset Template",
			JOptionPane.YES_NO_OPTION);
			
		if (result == JOptionPane.YES_OPTION) {
			boolean success = templateManager.resetToDefaultTemplate(templateManager.TemplateType.GOOD_MORAL);
			if (success) {
	...
	private void loadTemplate() {
		...
		if (result == JFileChooser.APPROVE_OPTION) {
			...
			if (templateManager.importCustomTemplate(templateManager.TemplateType.GOOD_MORAL, selected)) {
				templateFile = templateManager.getActiveTemplate(templateManager.TemplateType.GOOD_MORAL);
				selectedTemplateLoc.setText(templateFile.getAbsolutePath());
	...
	private void printSelectedStudents() {
		...
		modalPanel.showModal(studentsDataDAO, this, () -> {
			JOptionPane.showMessageDialog(this, "Certificate generation complete for " + selectedStudents.size() + " student(s).", "Generation Complete", JOptionPane.INFORMATION_MESSAGE);
		});
	}
}
