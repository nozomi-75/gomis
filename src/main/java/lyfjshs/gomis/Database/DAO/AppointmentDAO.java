...
public class AppointmentDAO {
    ...
    public boolean insertAppointment(int participantId, Integer guidanceCounselorId, String appointmentTitle,
                                     String appointmentType, Timestamp appointmentDateTime, String appointmentNotes,
                                     String appointmentStatus) {
        String query = "INSERT INTO APPOINTMENTS (participant_id, guidance_counselor_id, appointment_title, " +
                       "appointment_type, appointment_date_time, appointment_notes, appointment_status, updated_at) " +
                       "VALUES (?, ?, ?, ?, ?, ?, ?, NOW())";
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
            stmt.setInt(1, participantId);
            if (guidanceCounselorId != null) {
                stmt.setInt(2, guidanceCounselorId);
            } else {
    ...
    // UPDATE: Modify an existing appointment using an Appointment object
    public boolean updateAppointment(Appointment appointment) {
        String query = "UPDATE APPOINTMENTS SET participant_id = ?, guidance_counselor_id = ?, appointment_title = ?, " +
    ...
    // READ: Retrieve appointments for a specific date
    ...
    // Utility: Map ResultSet to Appointment object
    private Appointment mapResultSetToAppointment(ResultSet rs) throws SQLException {
        return new Appointment(
            rs.getInt("appointment_id"),
            rs.getInt("participant_id"),
            rs.getObject("guidance_counselor_id", Integer.class),
