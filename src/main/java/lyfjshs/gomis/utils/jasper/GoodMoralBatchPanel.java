...
public class GoodMoralBatchPanel extends JPanel {
	...
	public GoodMoralBatchPanel(Component parent, List<Student> students) {
		this.parent = parent;
	...
	private void setupUI() {
		// Create main panel with preview
		JPanel mainPanel = new JPanel(new MigLayout("", "[][grow]", "[grow]"));
		DatePicker datePicker = new DatePicker();
		datePicker.setSelectedDate(LocalDate.now());

		// Create content panel
		JPanel contentPanel = new JPanel(new MigLayout("fillx, insets 10", "[right][grow,fill]", "[100px][][][][][][100px][][][][][][]"));
		...
		// Students list setup
		setupStudentsList(contentPanel);

		// Preview button
		JButton previewBtn = new JButton("Preview Selected Student");
		previewBtn.addActionListener(e -> previewSelectedStudent());
		contentPanel.add(previewBtn, "cell 0 7 2 1,growx");
	...
	private void setupStudentsList(JPanel contentPanel) {
		JPanel studentsPanel = new JPanel(new MigLayout("fillx, insets 10", "[grow]", "[][grow]"));
		studentsPanel.setBorder(BorderFactory.createTitledBorder("Selected Students"));
		...
		contentPanel.add(studentsPanel, "cell 0 6 2 1,growx");
	}
	...
	private void setupImagesSection(JPanel contentPanel) {
		...
		contentPanel.add(toggleLogosBtn, "cell 0 11 2 1,growx");
		
		dropDownPanelLogosSection.setContent(imagesSection);
		contentPanel.add(dropDownPanelLogosSection, "cell 0 12 2 1,grow");
	...
	private void previewSelectedStudent() {
		int selectedIndex = studentList.getSelectedIndex();
		if (selectedIndex != -1) {
			Student selectedStudent = students.get(selectedIndex);
			updatePreview(selectedStudent);
		} else {
			showToast("Please select a student to preview", Toast.Type.WARNING);
		}
	}

	private void updatePreview(Student student) {
	...
	private void addFontNotes(JPanel contentPanel) {
		...
		contentPanel.add(fontNoteLabel, "cell 0 8 2 1,growx");
		JLabel fontNote2Label = new JLabel(" the final output due to system font availability.");
		contentPanel.add(fontNote2Label, "cell 0 9 2 1,growx");
	}
