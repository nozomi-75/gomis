...
public class S_Chartpanel extends JPanel {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getLogger(S_Chartpanel.class);
	private final ViolationDAO violationDAO;
	private final S_ReportIFilterPanel filterPanel;
	private final ReportChartContainer chartContainer;
	private final ReportTablePanel tablePanel;

	public S_Chartpanel(Connection connection) {
		...
		filterPanel = new S_ReportIFilterPanel();
	...
	private void updateDataAndRender(LocalDate start, LocalDate end, String violationType) {
		try {
			...
			logger.error("Error fetching data for small chart panel", e);
