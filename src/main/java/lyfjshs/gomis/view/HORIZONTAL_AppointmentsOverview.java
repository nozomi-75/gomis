...
public class HORIZONTAL_AppointmentsOverview extends JPanel {
    private static final Logger logger = LogManager.getLogger(HORIZONTAL_AppointmentsOverview.class);
    ...
    public HORIZONTAL_AppointmentsOverview(AppointmentDAO appointmentDAO, Connection conn) {
        ...
        datePickerPanel.add(datePicker, "cell 0 0,grow");

        // Add status filter combo box
        statusFilterComboBox = new JComboBox<>(new String[] {"All", "Scheduled", "In Progress", "Rescheduled", "Active", "Pending"});
        datePickerPanel.add(statusFilterComboBox, "cell 2 0");
        statusFilterComboBox.addActionListener(e -> refreshAppointments());
        ...
        datePickerPanel.add(scrollPane, "cell 1 0,grow");
        ...
        // Load today's appointments AFTER all components are initialized
        SwingUtilities.invokeLater(() -> {
            // Set the current date after all components are ready
    ...
    /**
     * Initialize the date picker with current date after all components are ready.
    ...
    private DatePicker createDatePicker() {
        ...
        // Remove picker.now() to prevent premature events during initialization
    ...
    private void showAppointmentDetailsPopup(Appointment appt) {
        ...
        try {
            ...
            logger.error("Error opening appointment details", e);
    ...
    private void loadAppointments(LocalDate date) {
        ...
        if (statusFilterComboBox != null) {
            loadAppointments(date, (String) statusFilterComboBox.getSelectedItem());
        } else {
            // Fallback to "All" if combo box is not yet initialized
            loadAppointments(date, "All");
        }
    }

    private void loadAppointments(LocalDate date, String statusFilter) {
        SwingUtilities.invokeLater(() -> {
                ...
                if (loadedAppointments != null) {
                    if (statusFilter == null || statusFilter.equalsIgnoreCase("All")) {
                        ...
                        loadedAppointments = loadedAppointments.stream()
                            .filter(appt -> {
                                String status = appt.getAppointmentStatus() != null ? appt.getAppointmentStatus().trim() : "";
                                return status.equalsIgnoreCase(statusFilter.trim());
                            })
                            .toList();
                    }
                }
    ...
    public void refreshAppointments() {
        ...
        if (statusFilterComboBox != null && datePicker != null) {
            loadAppointments(datePicker.getSelectedDate(), (String) statusFilterComboBox.getSelectedItem());
        }
    }
}
